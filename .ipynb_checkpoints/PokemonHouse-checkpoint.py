        ...
        for line in file:  # iterate over each line
            if not line.startswith('#') and not line.startswith('\n'):  # exclude parsing of # or new lines
                ...
                # parse doors
                    ...
                    temp_room = line[3]  # to hold name of the room to compare if we've already made a key for it in the dict.
                    if temp_room in doors:  # check to see if the room is already in the dict
                        doors[temp_room].append([line[0], line[4], line[2]])  # if so - append new door-pathway information onto the nested lists. ex: 'Bedroom': [['up1', 'Library', 'closed'], ['up2', 'Hall', 'open'], ['right', 'Bathroom', 'locked']]
                    else:
                        doors[line[3]] = [[line[0], line[4], line[2]]]  # else - makes new dict entry. ex: 'Bedroom': [['up1', 'Library', 'closed']]
